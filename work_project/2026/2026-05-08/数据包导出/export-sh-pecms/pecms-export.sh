...
ENCRYPT_PASSWORD="#Pecms@3957.,com!"
MYSQL_CNF="/root/.my.cnf"
MYSQL_CLIENT_CNF=""
MYSQL_DATABASE="${MYSQL_DATABASE:-}"

read_mysql_cnf_value() {
    local key=$1

    awk -v wanted="$key" '
        /^[[:space:]]*\[/ {
            in_client = ($0 ~ /^[[:space:]]*\[client\][[:space:]]*$/)
            next
        }
        in_client {
            line = $0
...
prepare_mysql_defaults_file() {
    MYSQL_CLIENT_CNF=$(mktemp /tmp/pecms-mysql-client.XXXXXX.cnf) || exit 1
    ...
    awk '
        ...
        in_client {
            ...
            if (key == "database" || key == "thread_handling") next
            print line
        }
    ' "$MYSQL_CNF" > "$MYSQL_CLIENT_CNF"

    if ! grep -q '^[[:space:]]*\[client\][[:space:]]*$' "$MYSQL_CLIENT_CNF"; then
        echo "ERROR: no [client] section found in $MYSQL_CNF" >&2
        exit 1
    fi
}

cleanup_mysql_defaults_file() {
    if [ -n "$MYSQL_CLIENT_CNF" ] && [ -f "$MYSQL_CLIENT_CNF" ]; then
        rm -f "$MYSQL_CLIENT_CNF"
    fi
}

mysql_scalar() {
    local defaults_file="${MYSQL_CLIENT_CNF:-$MYSQL_CNF}"
...
export_project() {
    local PROJECT_ID_PARAM=$1
    do_export "PROJECT" "$PROJECT_ID_PARAM"
}

export_contract() {
    local CONTRACT_NO_PARAM=$1
    do_export "CONTRACT" "$CONTRACT_NO_PARAM"
}
...
do_export() {
    ...
    local DOC_IDS=""
    local DB_NAME=""
    local EXPORT_TIME_LOG=""

    prepare_mysql_defaults_file
    trap cleanup_mysql_defaults_file EXIT

    PARAM_ID_SQL=$(sql_quote "$PARAM_ID")

    DB_NAME="${MYSQL_DATABASE:-$(read_mysql_cnf_value database)}"
    if [ -z "$DB_NAME" ] || [ "$DB_NAME" = "NULL" ]; then
        echo "ERROR: no database configured; set database=... in $MYSQL_CNF or MYSQL_DATABASE=..." >&2
        exit 1
    fi

    if [ "$TYPE" = "PROJECT" ]; then
        DISPATCH_IDS="SELECT DISTINCT id FROM project_dispatch WHERE projectId = ${PARAM_ID_SQL}"
        PROJECT_INFO=$(mysql --defaults-file="$MYSQL_CLIENT_CNF" --default-character-set=utf8mb4 -N -B "$DB_NAME" -e "
            SELECT id, name
            FROM project_management
            WHERE id = ${PARAM_ID_SQL}
            LIMIT 1;
        ")

        if [ -z "$PROJECT_INFO" ]; then
            echo "ERROR: project id not found: $PARAM_ID"
            exit 1
        fi

        PROJECT_NAME=$(echo "$PROJECT_INFO" | awk '{print $2}')
        if [ -z "$PROJECT_NAME" ]; then
            echo "WARN: empty project name, using fallback name"
            PROJECT_NAME="project_${PARAM_ID}"
        fi
    elif [ "$TYPE" = "CONTRACT" ]; then
        DISPATCH_IDS="SELECT DISTINCT id FROM project_dispatch WHERE supervisor_contract_no = ${PARAM_ID_SQL}"
        PROJECT_INFO=$(mysql --defaults-file="$MYSQL_CLIENT_CNF" --default-character-set=utf8mb4 -N -B "$DB_NAME" -e "
            SELECT id, code
            FROM supervision_department_contract
            WHERE code = ${PARAM_ID_SQL}
            LIMIT 1;
        ")

        if [ -z "$PROJECT_INFO" ]; then
            echo "ERROR: contract not found: $PARAM_ID"
            exit 1
        fi

        PROJECT_NAME="contract_${PARAM_ID}"
    else
        echo "ERROR: invalid argument"
        exit 1
    fi
    DOC_IDS="SELECT DISTINCT f.id FROM yw_supervision_file f WHERE f.parent_id IN (SELECT p.id FROM yw_supervision_file_parent p WHERE p.dispatch_id IN (${DISPATCH_IDS}))"

    # file business ID source SQLs
    local FILE_BIZID_SOURCE_SQLS=()

    # sanitize export directory name
    ...
    EXPORT_TIME_LOG="${EXPORT_DIR}/export_time.tsv"
    printf "table\tstatus\tseconds\tbytes\n" > "$EXPORT_TIME_LOG"
    # ==============================================
    # 公共导出函数（内部使用外层变量）
    # ==============================================
    to_export() {
        ...
        local START_TS END_TS COST FILE_BYTES

        START_TS=$(date +%s)
        echo "-> Export table: ${TABLE_NAME}"

        mysqldump --defaults-file="$MYSQL_CLIENT_CNF" \
            --default-character-set=utf8mb4 \
            --single-transaction \
            --quick \
            --skip-lock-tables \
            ...
            "$DB_NAME" "$TABLE_NAME" > "$FILE_PATH" 2>"$ERR_PATH"

        if [ $? -ne 0 ]; then
            echo "  -> Export failed, skipped: ${TABLE_NAME}"
            if [ -s "$ERR_PATH" ]; then
                sed -n '1p' "$ERR_PATH"
            fi
            ...
            printf "%s\tfailed\t%s\t0\n" "$TABLE_NAME" "$COST" >> "$EXPORT_TIME_LOG"
            return 0
        fi
        rm -f "$ERR_PATH"

        if ! grep -q '^INSERT INTO ' "$FILE_PATH"; then
            rm -f "$FILE_PATH"
            END_TS=$(date +%s)
            COST=$((END_TS - START_TS))
            printf "%s\tempty\t%s\t0\n" "$TABLE_NAME" "$COST" >> "$EXPORT_TIME_LOG"
        ...
        END_TS=$(date +%s)
        COST=$((END_TS - START_TS))
        echo "  -> Done: ${TABLE_NAME}, ${FILE_BYTES} bytes, ${COST}s"
        printf "%s\tok\t%s\t%s\n" "$TABLE_NAME" "$COST" "$FILE_BYTES" >> "$EXPORT_TIME_LOG"
    }
    export_table() {
        local TABLE_NAME=$1
        local WHERE_CONDITION=$2
        ...
        if [ -n "$SYS_FILE_FLAG" ] && [ "$SYS_FILE_FLAG" -eq 1 ]; then
            FILE_BIZID_SOURCE_SQLS+=("SELECT id FROM ${TABLE_NAME} WHERE ${WHERE_CONDITION}")
        fi
    }

    build_file_bizid_sql() {
        local union_sql=""
        local sql

        for sql in "${FILE_BIZID_SOURCE_SQLS[@]}"; do
            if [ -z "$union_sql" ]; then
                union_sql="$sql"
            else
                union_sql="${union_sql} UNION ALL ${sql}"
            fi
        done

        if [ -n "$union_sql" ]; then
            printf "SELECT DISTINCT id FROM (%s) file_biz_ids" "$union_sql"
        fi
    }

    export_by_ids() {
        local ids
        ids=$(build_file_bizid_sql)
        ...
        local ATTACH_LIST="${EXPORT_DIR}/attachment_files.txt"
        echo -e "\n==> Preparing attachment list..."
        mysql --defaults-file="$MYSQL_CLIENT_CNF" --default-character-set=utf8mb4 -N -B "$DB_NAME" -e "
            ...
            WHERE u.biz_id IN (${ids})
              ...
              AND f.file_path <> '';
        " | sed 's#\\#/#g; s#^[\\/]*##' | sort -u > "$ATTACH_LIST"

        if [ ! -s "$ATTACH_LIST" ]; then
            echo "==> No attachment path found"
            rm -f "$ATTACH_LIST"
            return 0
        fi
        ...
        if command -v rsync >/dev/null 2>&1; then
            rsync -a --ignore-missing-args --files-from="$ATTACH_LIST" "${FILE_ROOT%/}/" "$BACKUP_FILE_DIR/"
        else
            while read -r rel_path; do
                [ -z "$rel_path" ] && continue
                local real_file="${FILE_ROOT%/}/${rel_path}"
                local dest_file="${BACKUP_FILE_DIR}/${rel_path}"
                local dest_dir
                dest_dir=$(dirname "$dest_file")
                mkdir -p "$dest_dir"
                if [ -f "$real_file" ]; then
                    cp -p "$real_file" "$dest_file"
                else
                    echo "Missing file: $real_file"
                fi
            done < "$ATTACH_LIST"
        fi

        echo -e "\n==> Attachment export completed"
    }
    export_group() {
        ...
        local primary_ids
        primary_ids="SELECT DISTINCT $mparam FROM $primary_table WHERE $main_cond"
        local secondary_cond="$param IN (${primary_ids})"
        local secondary_ids=""

        for sec in $secondary_tables; do
            export_table "$sec" "$secondary_cond" 0
            local sec_ids
            sec_ids="SELECT DISTINCT id FROM $sec WHERE $secondary_cond"
            if [ -n "$secondary_ids" ]; then
                secondary_ids="${secondary_ids} UNION ALL ${sec_ids}"
            else
        ...
        if [ -n "$tertiary_tables" ] && [ -n "$secondary_ids" ]; then
            local tertiary_cond="$param IN (SELECT DISTINCT id FROM (${secondary_ids}) secondary_ids)"
    ...
    echo "========================================"
    if [ "$TYPE" = "PROJECT" ]; then
        echo "Export PROJECT ID: $PARAM_ID"
    elif [ "$TYPE" = "CONTRACT" ]; then
        echo "Export CONTRACT NUMBER: $PARAM_ID"
    else
        echo "ERROR: invalid argument"
        exit 1
    fi
    ...
    # business tables
    if [ "$TYPE" = "PROJECT" ]; then
        export_table "project_management" "id=${PARAM_ID_SQL}" 0
        export_table "project_apparatus" "project_id=${PARAM_ID_SQL}" 0
        export_table "project_dispatch" "projectId=${PARAM_ID_SQL}" 0
    elif [ "$TYPE" = "CONTRACT" ]; then
        export_table "supervision_department_contract" "code=${PARAM_ID_SQL}" 0
        export_table "project_dispatch" "supervisor_contract_no=${PARAM_ID_SQL}" 0
        export_table "project_management" "id IN (SELECT projectid FROM project_dispatch WHERE supervisor_contract_no=${PARAM_ID_SQL})" 0
        export_table "project_apparatus" "project_id IN (SELECT projectid FROM project_dispatch WHERE supervisor_contract_no=${PARAM_ID_SQL})" 0
    else
        echo "ERROR: invalid argument"
        exit 1
    fi

    export_table "project_dispatch_dispatch_user" "project_dispatch_id IN (${DISPATCH_IDS})" 0
    export_table "project_dispatch_actual_user" "project_dispatch_id IN (${DISPATCH_IDS})" 0
    export_table "project_dispatch_client_user" "project_dispatch_id IN (${DISPATCH_IDS})" 0
    export_table "project_dispatch_represent_user" "project_dispatch_id IN (${DISPATCH_IDS})" 0
    export_table "project_dispatch_supplier_user" "project_dispatch_id IN (${DISPATCH_IDS})" 0
    export_table "dispatch_change" "dispatchid IN (${DISPATCH_IDS})" 0
    export_table "dispatch_signin" "dispatch_id IN (${DISPATCH_IDS})" 0
    export_table "review_dispatch" "process_instance_id IN (SELECT process_instance_id from project_dispatch WHERE id IN (${DISPATCH_IDS}))" 0
    ...
    export_table "dispatch_no_change" "dispatch_id IN (${DISPATCH_IDS})" 0
    ...
    local YW_WEEKLY_COND="SELECT DISTINCT id FROM yw_supervision_weekly WHERE dispatch_id IN (${DISPATCH_IDS})"
    export_table "sys_weekly" "documentid IN ($YW_WEEKLY_COND)" 0
    local SYS_WEEKLY_COND="SELECT DISTINCT id FROM sys_weekly WHERE documentid IN ($YW_WEEKLY_COND)"
    ...
    #执行状态
    DISPATCH_EXECUTION_ID="SELECT DISTINCT id FROM dispatch_execution WHERE dispatch_id IN (${DISPATCH_IDS}) AND del_flag=0"
...
#    export_table "dispatch_execution_actual_user" "dispatch_execution_id IN ($DISPATCH_EXECUTION_ID)" 0
    ...
    #人员交接
    HANDOVER_ID="SELECT DISTINCT id FROM yw_handover_main WHERE dispatchid IN (${DISPATCH_IDS}) AND del_flag=0"
    ...
    FILE_PARENT_ID="SELECT DISTINCT id FROM yw_supervision_file_parent WHERE dispatch_id IN (${DISPATCH_IDS}) AND del_flag=0"
    ...
    YW_CATALOGUE_ID="SELECT DISTINCT yw_catalogue_id FROM yw_supervision_file WHERE parent_id IN ($FILE_PARENT_ID) AND del_flag=0"
    export_table "yw_supervision_catalogue" "id IN ($YW_CATALOGUE_ID) AND del_flag=0" 1
    #录入细则必选表单
    DEVICE_ID="SELECT DISTINCT id FROM project_device_management WHERE dispatchid IN (${DISPATCH_IDS}) AND del_flag=0"
    ...
    ZYCS_ZDSB_ID="SELECT DISTINCT ID FROM rep_sbzyjscsjg_zdsb WHERE deviceid IN ($DEVICE_ID)"
    ...
    ZYCS_JSB_ID="SELECT DISTINCT ID FROM rep_sbzyjscsjg_jsb WHERE deviceid IN ($DEVICE_ID)"
    ...
    ZYCS_F_ZDSB_ID="SELECT DISTINCT ID FROM rep_zyjscsjg_sb WHERE deviceid IN ($DEVICE_ID)"
    ...
    CONTROL_POINTS_ID="SELECT DISTINCT ID FROM rep_control_points WHERE deviceid IN ($DEVICE_ID)"
    ...
    YW_JLDG_ZTNR_SBQD_ID="SELECT DISTINCT id FROM rep_jldg_ztnr WHERE docid IN (${DOC_IDS})"
    ...
    JLDG_JZJSYJH_PID="SELECT DISTINCT id FROM rep_jzjsyjh WHERE docid IN (${DOC_IDS})"
    ...
    #监理日志-问题及解决
    YW_SUPERVISION_LOG_ID="SELECT DISTINCT id FROM yw_supervision_log WHERE dispatch_id IN (${DISPATCH_IDS}) AND del_flag=0"
    ...
    #监理日志-报表
    ttfdjzyhxjyjzjlb_data_id="SELECT DISTINCT id FROM rep_ttfdjzyhxjyjzjlb_data_daily WHERE docid IN (${DOC_IDS})"
    ...
    tt_fd_jyjl_id="SELECT DISTINCT id FROM rep_tt_fd_jyjl_daily WHERE docid IN (${DOC_IDS})"
    tt_fd_jyjl_pid="SELECT DISTINCT id FROM rep_tt_fd_jyjl_list_daily WHERE pid IN ($tt_fd_jyjl_id)"
    ...
    fdtqztzzjcjlb_id="SELECT DISTINCT id FROM rep_fdtqztzzjcjlb_data_daily WHERE docid IN (${DOC_IDS})"
    ...
    bzcbcx_dctjbz_data_id="SELECT DISTINCT id FROM rep_bzcbcx_dctjbz_data_daily WHERE docid IN (${DOC_IDS})"
    ...
    bzcbcx_dcztbz_data_id="SELECT DISTINCT id FROM rep_bzcbcx_dctjbz_data_daily WHERE docid IN (${DOC_IDS})"
    ...
    ecnjyzzccjcjl_id="SELECT DISTINCT id FROM rep_ecnjyzzccjcjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    basics_test_id="SELECT DISTINCT id FROM rep_basics_test_daily WHERE docid IN (${DOC_IDS})"
    ...
    zzjyjzjlb_data_id="SELECT DISTINCT id FROM rep_zzjyjzjlb_data_daily WHERE docid IN (${DOC_IDS})"
    ...
    withstand_appearance_id="SELECT DISTINCT id FROM rep_withstand_appearance_daily WHERE docid IN (${DOC_IDS})"
    ...
    glbzjnysyjyjl_id="SELECT DISTINCT id FROM rep_glbzjnysyjyjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    sand_blasting_paint_id="SELECT DISTINCT id FROM rep_sand_blasting_paint_daily WHERE docid IN (${DOC_IDS})"
    ...
    packing_record_id="SELECT DISTINCT id FROM rep_packing_record_daily WHERE docid IN (${DOC_IDS})"
    ...
    completion_data_id="SELECT DISTINCT id FROM rep_completion_data_daily WHERE docid IN (${DOC_IDS})"
    ...
    other_check_id="SELECT DISTINCT id FROM rep_other_check_daily WHERE docid IN (${DOC_IDS})"
    ...
    tzgywjscjl_id="SELECT DISTINCT id FROM rep_tzgywjscjl_daily WHERE docid IN (${DOC_IDS})"
    tzgywjscjl_pid="SELECT DISTINCT id FROM rep_tzgywjscjl_list_daily WHERE pid IN ($tzgywjscjl_id)"
    ...
    welding_technology_id="SELECT DISTINCT id FROM rep_welding_technology_daily WHERE docid IN (${DOC_IDS})"
    ...
    material_quality_id="SELECT DISTINCT id FROM rep_material_quality_daily WHERE docid IN (${DOC_IDS})"
    ...
    incoming_plate_id="SELECT DISTINCT id FROM rep_incoming_plate_daily WHERE docid IN (${DOC_IDS})"
    incoming_plate_pid="SELECT DISTINCT id FROM rep_incoming_plate_list_daily WHERE pid IN ($incoming_plate_id)"
    ...
    incoming_pipe_id="SELECT DISTINCT id FROM rep_incoming_pipe_daily WHERE docid IN (${DOC_IDS})"
    incoming_pipe_pid="SELECT DISTINCT id FROM rep_incoming_pipe_list_daily WHERE pid IN ($incoming_pipe_id)"
    ...
    incoming_forging_id="SELECT DISTINCT id FROM rep_incoming_forging_daily WHERE docid IN (${DOC_IDS})"
    incoming_forging_pid="SELECT DISTINCT id FROM rep_incoming_forging_list_daily WHERE pid IN ($incoming_forging_id)"
    ...
    incoming_hc_id="SELECT DISTINCT id FROM rep_incoming_hc_daily WHERE docid IN (${DOC_IDS})"
    incoming_hc_pid="SELECT DISTINCT id FROM rep_incoming_hc_list_daily WHERE pid IN ($incoming_hc_id)"
    ...
    retest_material_id="SELECT DISTINCT id FROM rep_retest_material_daily WHERE docid IN (${DOC_IDS})"
    retest_material_pid="SELECT DISTINCT id FROM rep_retest_material_list WHERE pid IN ($retest_material_id)"
    ...
    xljyjl_id="SELECT DISTINCT id FROM rep_xljyjl_daily WHERE docid IN (${DOC_IDS})"
    xljyjl_pid="SELECT DISTINCT id FROM rep_xljyjl_list_daily WHERE pid IN ($xljyjl_id)"
    ...
    tjjyjzjl_id="SELECT DISTINCT id FROM rep_tjjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    tjjyjzjl_pid="SELECT DISTINCT id FROM rep_tjjyjzjl_list_daily WHERE pid IN ($tjjyjzjl_id)"
    ...
    cxftjyjzjl_id="SELECT DISTINCT id FROM rep_cxftjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    cxztjyjzjl_id="SELECT DISTINCT id FROM rep_cxztjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    flange_check_id="SELECT DISTINCT id FROM rep_flange_check_daily WHERE docid IN (${DOC_IDS})"
    ...
    cxgjjyjzjlb_id="SELECT DISTINCT id FROM rep_cxgjjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    cxgjjyjzjlb_pid="SELECT DISTINCT id FROM rep_cxgjjyjzjlb_list_daily WHERE pid IN ($cxgjjyjzjlb_id)"
    ...
    uxhrgcpjyjzjl_id="SELECT DISTINCT id FROM rep_uxhrgcpjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    uxhrgcpjyjzjl_pid="SELECT DISTINCT id FROM rep_uxhrgcpjyjzjl_list_daily WHERE pid IN ($uxhrgcpjyjzjl_id)"
    ...
    gbcpjyjzjl_id="SELECT DISTINCT id FROM rep_gbcpjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    gbcpjyjzjl_pid="SELECT DISTINCT id FROM rep_gbcpjyjzjl_list_daily WHERE pid IN ($gbcpjyjzjl_id)"
    ...
    zlbcpjyjzjl_id="SELECT DISTINCT id FROM rep_zlbcpjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    zlbcpjyjzjl_pid="SELECT DISTINCT id FROM rep_zlbcpjyjzjl_list_daily WHERE pid IN ($zlbcpjyjzjl_id)"
    ...
    fbftjyjzjlb_data_id="SELECT DISTINCT id FROM rep_fbftjyjzjlb_data_daily WHERE docid IN (${DOC_IDS})"
    ...
    fpztjyjzjlb_data_id="SELECT DISTINCT id FROM rep_fpztjyjzjlb_data_daily WHERE docid IN (${DOC_IDS})"
    ...
    ylrqjyjlb_id="SELECT DISTINCT id FROM rep_ylrqjyjlb_daily WHERE docid IN (${DOC_IDS})"
    ...
    wxwgjrcjyjlb_id="SELECT DISTINCT id FROM rep_wxwgjrcjyjlb_daily WHERE docid IN (${DOC_IDS})"
    wxwgjrcjyjlb_pid="SELECT DISTINCT id FROM rep_wxwgjrcjyjlb_list_daily WHERE pid IN ($wxwgjrcjyjlb_id)"
    ...
    wgjjcjz_id="SELECT DISTINCT id FROM rep_wgjjcjz_daily WHERE docid IN (${DOC_IDS})"
    export_table "rep_wgjjcjz_daily" "id IN ($wgjjcjz_id)" 0 #报表-外购件检查见证表
    export_table "rep_wgjjcjz_list_daily" "pid IN ($wgjjcjz_id)" 0

    hjjyjl_id="SELECT DISTINCT id FROM rep_hjjyjl_daily WHERE docid IN (${DOC_IDS})"
    export_table "rep_hjjyjl_daily" "id IN ($hjjyjl_id)" 0 #报表-焊接过程检验记录表
    export_table "rep_hjjyjl_list_daily" "pid IN ($hjjyjl_id)" 0

    jkhxkkjzjk_id="SELECT DISTINCT id FROM rep_jkhxkkjzjk_daily WHERE docid IN (${DOC_IDS})"
    ...
    jgzzjyjzjl_id="SELECT DISTINCT id FROM rep_jgzzjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    gszzjcxjyjzjl_data_id="SELECT DISTINCT id FROM rep_gszzjcxjyjzjl_data_daily WHERE docid IN (${DOC_IDS})"
    ...
    hrgygbzjjyjzjlb_data_id="SELECT DISTINCT id FROM rep_hrgygbzjjyjzjlb_data_daily WHERE docid IN (${DOC_IDS})"
    ...
    tnjccjcjl_id="SELECT DISTINCT id FROM rep_tnjccjcjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    yhjjyjzjl_id="SELECT DISTINCT id FROM rep_yhjjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    zzjcjz_id="SELECT DISTINCT id FROM rep_zzjcjz_daily WHERE docid IN (${DOC_IDS})"
    export_table "rep_zzjcjz_daily" "id IN ($zzjcjz_id)" 0 #报表-总装检查见证表
    export_table "rep_zzjcjz_list_daily" "pid IN ($zzjcjz_id)" 0

    wgjccjcjl_id="SELECT DISTINCT id FROM rep_wgjccjcjl_daily WHERE docid IN (${DOC_IDS})"
    wgjccjcjl_pid="SELECT DISTINCT id FROM rep_wgjccjcjl_list_daily WHERE pid IN ($wgjccjcjl_id)"
    ...
    sjjyjl_id="SELECT DISTINCT id FROM rep_sjjyjl_daily WHERE docid IN (${DOC_IDS})"
    sjjyjl_pid="SELECT DISTINCT id FROM rep_sjjyjl_list_daily WHERE pid IN ($sjjyjl_id)"
    ...
    radial_id="SELECT DISTINCT id FROM rep_radial_daily WHERE docid IN (${DOC_IDS})"
    export_table "rep_radial_daily" "id IN ($radial_id)" 0 #报表-射线检验记录表
    export_table "rep_radial_list_daily" "pid IN ($radial_id)" 0

    ultrasonic_id="SELECT DISTINCT id FROM rep_ultrasonic_daily WHERE docid IN (${DOC_IDS})"
    ...
    infiltration_id="SELECT DISTINCT id FROM rep_infiltration_daily WHERE docid IN (${DOC_IDS})"
    ...
    wsjcjl_id="SELECT DISTINCT id FROM rep_wsjcjl_daily WHERE docid IN (${DOC_IDS})"
    export_table "rep_wsjcjl_daily" "id IN ($wsjcjl_id)" 0 #报表-无损检测记录表
    export_table "rep_wsjcjl_list_daily" "pid IN ($wsjcjl_id)" 0

    rcljyjla_id="SELECT DISTINCT id FROM rep_rcljyjla_daily WHERE docid IN (${DOC_IDS})"
    ...
    heat_treatment_id="SELECT DISTINCT id FROM rep_heat_treatment_daily WHERE docid IN (${DOC_IDS})"
    ...
    slsyjyjlb_id="SELECT DISTINCT id FROM rep_slsyjyjlb_daily WHERE docid IN (${DOC_IDS})"
    ...
    ydjyjlb_check_id="SELECT DISTINCT id FROM rep_ydjyjlb_check_daily WHERE docid IN (${DOC_IDS})"
    ...
    rotor_runout_id="SELECT DISTINCT id FROM rep_rotor_runout_daily WHERE docid IN (${DOC_IDS})"
    ...
    djphsyjz_b_id="SELECT DISTINCT id FROM rep_djphsyjz_b_daily WHERE docid IN (${DOC_IDS})"
    ...
    zzdphjcssyjz_id="SELECT DISTINCT id FROM rep_zzdphjcssyjz_daily WHERE docid IN (${DOC_IDS})"
    export_table "rep_zzdphjcssyjz_daily" "id IN ($zzdphjcssyjz_id)" 0 #报表-转子动平衡及超速试验见证表

    scjdtljcb_id="SELECT DISTINCT id FROM rep_scjdtljcb_daily WHERE docid IN (${DOC_IDS})"
    ...
    ylcssyjzjl_id="SELECT DISTINCT id FROM rep_ylcssyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    incoming_casting_id="SELECT DISTINCT id FROM rep_incoming_casting_daily WHERE docid IN (${DOC_IDS})"
    incoming_casting_pid="SELECT DISTINCT id FROM rep_incoming_casting_list_daily WHERE pid IN ($incoming_casting_id)"
    ...
    incoming_profile_id="SELECT DISTINCT id FROM rep_incoming_profile_daily WHERE docid IN (${DOC_IDS})"
    ...
    bxpzjcpjyjlb_id="SELECT DISTINCT id FROM rep_bxpzjcpjyjlb_daily WHERE docid IN (${DOC_IDS})"
    bxpzjcpjyjlb_pid="SELECT DISTINCT id FROM rep_bxpzjcpjyjlb_list_daily WHERE pid IN ($bxpzjcpjyjlb_id)"
    ...
    gsbcpjyjzjlb_id="SELECT DISTINCT id FROM rep_gsbcpjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    gsbcpjyjzjlb_pid="SELECT DISTINCT id FROM rep_gsbcpjyjzjlb_list_daily WHERE pid IN ($gsbcpjyjzjlb_id)"
    ...
    cpgcpjyjzjlb_id="SELECT DISTINCT id FROM rep_cpgcpjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    cpgcpjyjzjlb_pid="SELECT DISTINCT id FROM rep_cpgcpjyjzjlb_list_daily WHERE pid IN ($cpgcpjyjzjlb_id)"
    ...
    bpjyjzjlb_id="SELECT DISTINCT id FROM rep_bpjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    bpjyjzjlb_pid="SELECT DISTINCT id FROM rep_bpjyjzjlb_daily WHERE pid IN ($bpjyjzjlb_id)"
    ...
    dygjyjzjlb_id="SELECT DISTINCT id FROM rep_dygjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    dygjyjzjlb_pid="SELECT DISTINCT id FROM rep_dygjyjzjlb_list_daily WHERE pid IN ($dygjyjzjlb_id)"
    ...
    fpcxjyjzjlb_id="SELECT DISTINCT id FROM rep_dygjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    fpcxjyjzjlb_pid="SELECT DISTINCT id FROM rep_fpcxjyjzjlb_list_daily WHERE pid IN ($fpcxjyjzjlb_id)"
    ...
    fpcxjyjzjlb_id="SELECT DISTINCT id FROM rep_fpzzjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    fpcxjyjzjlb_pid="SELECT DISTINCT id FROM rep_fpzzjyjzjlb_list WHERE pid IN ($fpcxjyjzjlb_id)"
    ...
    pgjyjzjlb_id="SELECT DISTINCT id FROM rep_pgjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    pgjyjzjlb_pid="SELECT DISTINCT id FROM rep_pgjyjzjlb_list_daily WHERE pid IN ($pgjyjzjlb_id)"
    ...
    lxjzgjyjzjl_id="SELECT DISTINCT id FROM rep_lxjzgjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    lxjzgjyjzjl_pid="SELECT DISTINCT id FROM rep_lxjzgjyjzjl_list_daily WHERE pid IN ($lxjzgjyjzjl_id)"
    ...
    zxtssjjyjzjl_id="SELECT DISTINCT id FROM rep_zxtssjjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    zxtssjjyjzjl_pid="SELECT DISTINCT id FROM rep_zxtssjjyjzjl_list_daily WHERE pid IN ($zxtssjjyjzjl_id)"
    ...
    zzypcpjyjzjl_id="SELECT DISTINCT id FROM rep_zzypcpjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    zzypcpjyjzjl_pid="SELECT DISTINCT id FROM rep_zzypcpjyjzjl_list_daily WHERE pid IN ($zzypcpjyjzjl_id)"
    ...
    fsddpjyjzjl_id="SELECT DISTINCT id FROM rep_fsddpjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    fsdyzzjyjzjl_id="SELECT DISTINCT id FROM rep_fsdyzzjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    dldmkjyjzjl_id="SELECT DISTINCT id FROM rep_dldmkjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    dldyzzjyjzjl_id="SELECT DISTINCT id FROM rep_dldyzzjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    ydycfdjyjzjl_id="SELECT DISTINCT id FROM rep_ydycfdjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    zcljcjl_id="SELECT DISTINCT id FROM rep_zcljcjl_daily WHERE docid IN (${DOC_IDS})"
    export_table "rep_zcljcjl_daily" "id IN ($zcljcjl_id)" 0 #报表-砖衬里检查记录表
    export_table "rep_zcljcjl_list_daily" "pid IN ($zcljcjl_id)" 0

    tcqwlcljcjl_id="SELECT DISTINCT id FROM rep_tcqwlcljcjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    zxjyjzjl_id="SELECT DISTINCT id FROM rep_zxjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    zxjyjzjl_pid="SELECT DISTINCT id FROM rep_zxjyjzjl_list_daily WHERE pid IN ($zxjyjzjl_id)"
    ...
    zfzzjyjzjl_id="SELECT DISTINCT id FROM rep_zfzzjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    zfzzjyjzjl_pid="SELECT DISTINCT id FROM rep_zfzzjyjzjl_list_daily WHERE pid IN ($zfzzjyjzjl_id)"
    ...
    rgtjyjzjl_id="SELECT DISTINCT id FROM rep_rgtjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    rgtjyjzjl_pid="SELECT DISTINCT id FROM rep_rgtjyjzjl_list_daily WHERE pid IN ($rgtjyjzjl_id)"
    ...
    jxlxjyjzjl_id="SELECT DISTINCT id FROM rep_jxlxjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    jxlxjyjzjl_pid="SELECT DISTINCT id FROM rep_jxlxjyjzjl_list_daily WHERE pid IN ($jxlxjyjzjl_id)"
    ...
    fdsbzzjyjzjl_id="SELECT DISTINCT id FROM rep_fdsbzzjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    fdsbzzjyjzjl_pid="SELECT DISTINCT id FROM rep_fdsbzzjyjzjl_list_daily WHERE pid IN ($fdsbzzjyjzjl_id)"
    ...
    jtzlzjzzzzjy_id="SELECT DISTINCT id FROM rep_jtzlzjzzzzjy_daily WHERE docid IN (${DOC_IDS})"
    ...
    mbjyjzjl_id="SELECT DISTINCT id FROM rep_mbjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    mbjyjzjl_pid="SELECT DISTINCT id FROM rep_mbjyjzjl_list_daily WHERE pid IN ($mbjyjzjl_id)"
    ...
    mbjyjzjl_id="SELECT DISTINCT id FROM rep_qkbzzjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    qgzzjyjz_id="SELECT DISTINCT id FROM rep_qgzzjyjz_daily WHERE docid IN (${DOC_IDS})"
    qgzzjyjz_pid="SELECT DISTINCT id FROM rep_qgzzjyjz_list_daily WHERE id IN ($qgzzjyjz_id)"
    ...
    zzjyjzjl_id="SELECT DISTINCT id FROM rep_zzjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    zzjyjzjl_pid="SELECT DISTINCT id FROM rep_zzjyjzjl_list_daily WHERE pid IN ($zzjyjzjl_id)"
    ...
    qgrcl_id="SELECT DISTINCT zid FROM rep_qgrcl_daily WHERE docid IN (${DOC_IDS})"
    export_table "rep_qgrcl_daily" "zid IN ($qgrcl_id)" 0 #报表-球罐热处理检验记录表
    export_table "rep_qgrcl_list_daily" "pid IN ($qgrcl_id)" 0

    ztyzzjcjzjl_id="SELECT DISTINCT id FROM rep_ztyzzjcjzjl_daily WHERE docid IN (${DOC_IDS})"
    ztyzzjcjzjl_pid="SELECT DISTINCT id FROM rep_ztyzzjcjzjl_list_daily WHERE pid IN ($ztyzzjcjzjl_id)"
    ...
    zongzjyjzjl_id="SELECT DISTINCT id FROM rep_zongzjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    zongzjyjzjl_pid="SELECT DISTINCT id FROM rep_zongzjyjzjl_list_daily WHERE pid IN ($zongzjyjzjl_id)"
    ...
    jbzzyzzjkzscjcjz_id="SELECT DISTINCT id FROM rep_jbzzyzzjkzscjcjz_daily WHERE docid IN (${DOC_IDS})"
    jbzzyzzjkzscjcjz_pid="SELECT DISTINCT id FROM rep_jbzzyzzjkzscjcjz_list_daily WHERE pid IN ($jbzzyzzjkzscjcjz_id)"
    ...
    pgjyjzjl_id="SELECT DISTINCT id FROM rep_pgjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    pgjyjzjl_pid="SELECT DISTINCT id FROM rep_pgjyjzjl_list_daily WHERE pid IN ($pgjyjzjl_id)"
    ...
    psptjcjzjl_id="SELECT DISTINCT id FROM rep_psptjcjzjl_daily WHERE docid IN (${DOC_IDS})"
    psptjcjzjl_pid="SELECT DISTINCT id FROM rep_psptjcjzjl_list_daily WHERE pid IN ($psptjcjzjl_id)"
    ...
    jgjrcjyjzjlb_id="SELECT DISTINCT id FROM rep_jgjrcjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    jgjrcjyjzjlb_pid="SELECT DISTINCT id FROM rep_jgjrcjyjzjlb_list_daily WHERE pid IN ($jgjrcjyjzjlb_id)"
    ...
    ftyzjyjzjlb_id="SELECT DISTINCT id FROM rep_ftyzjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    ftyzjyjzjlb_pid="SELECT DISTINCT id FROM rep_ftyzjyjzjlb_list_daily WHERE pid IN ($ftyzjyjzjlb_id)"
    ...
    tjjjgjyjzjl_id="SELECT DISTINCT id FROM rep_tjjjgjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    tjjjgjyjzjl_pid="SELECT DISTINCT id FROM rep_tjjjgjyjzjl_list_daily WHERE pid IN ($tjjjgjyjzjl_id)"
    ...
    ttjjgjyjzjl_id="SELECT DISTINCT id FROM rep_ttjjgjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ttjjgjyjzjl_pid="SELECT DISTINCT id FROM rep_ttjjgjyjzjl_list_daily WHERE pid IN ($ttjjgjyjzjl_id)"
    ...
    ysdjyjzjl_id="SELECT DISTINCT id FROM rep_ysdjyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ysdjyjzjl_pid="SELECT DISTINCT id FROM rep_ysdjyjzjl_list_daily WHERE id IN ($ysdjyjzjl_id)"
    ...
    gxjyjzjlb_id="SELECT DISTINCT id FROM rep_gxjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    gxjyjzjlb_pid="SELECT DISTINCT id FROM rep_gxjyjzjlb_list_daily WHERE id IN ($gxjyjzjlb_id)"
    ...
    byccpjyjzjlb_id="SELECT DISTINCT id FROM rep_byccpjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    byccpjyjzjlb_pid="SELECT DISTINCT id FROM rep_byccpjyjzjlb_list_daily WHERE pid IN ($byccpjyjzjlb_id)"
    ...
    jgcpjyjzjlb_id="SELECT DISTINCT id FROM rep_jgcpjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    jgcpjyjzjlb_pid="SELECT DISTINCT id FROM rep_jgcpjyjzjlb_list_daily WHERE pid IN ($jgcpjyjzjlb_id)"
    ...
    fjjyjzjlb_id="SELECT DISTINCT id FROM rep_fjjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    fjjyjzjlb_pid="SELECT DISTINCT id FROM rep_fjjyjzjlb_list_daily WHERE pid IN ($fjjyjzjlb_id)"
    ...
    txthljyjl_id="SELECT DISTINCT id FROM rep_txthljyjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    jrgjyjlb_id="SELECT DISTINCT id FROM rep_jrgjyjlb_daily WHERE docid IN (${DOC_IDS})"
    jrgjyjlb_pid="SELECT DISTINCT id FROM rep_jrgjyjlb_list_daily WHERE pid IN ($jrgjyjlb_id)"
    ...
    dhchdjyjl_id="SELECT DISTINCT id FROM rep_dhchdjyjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    sxgfyjyjlb_id="SELECT DISTINCT id FROM rep_sxgfyjyjlb_daily WHERE docid IN (${DOC_IDS})"
    sxgfyjyjlb_pid="SELECT DISTINCT id FROM rep_sxgfyjyjlb_list_daily WHERE pid IN ($sxgfyjyjlb_id)"
    ...
    sxgccwgjyjlb_id="SELECT DISTINCT id FROM rep_sxgccwgjyjlb_daily WHERE docid IN (${DOC_IDS})"
    sxgccwgjyjlb_pid="SELECT DISTINCT id FROM rep_sxgccwgjyjlb_list_daily WHERE pid IN ($sxgccwgjyjlb_id)"
    ...
    wgccwgjyjl_id="SELECT DISTINCT id FROM rep_wgccwgjyjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    sxgjgptqsyjzjl_id="SELECT DISTINCT id FROM rep_sxgjgptqsyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    sxgjgpnyjzjl_id="SELECT DISTINCT id FROM rep_sxgjgpnyjzjl_daily WHERE docid IN (${DOC_IDS})"
    ...
    gjglzccwgjyjlb_id="SELECT DISTINCT id FROM rep_gjglzccwgjyjlb_daily WHERE docid IN (${DOC_IDS})"
    gjglzccwgjyjlb_pid="SELECT DISTINCT id FROM rep_gjglzccwgjyjlb_list_daily WHERE pid IN ($gjglzccwgjyjlb_id)"
    ...
    gjgdblccwgjyjlb_id="SELECT DISTINCT id FROM rep_gjgdblccwgjyjlb_daily WHERE docid IN (${DOC_IDS})"
    gjgdblccwgjyjlb_pid="SELECT DISTINCT id FROM rep_gjgdblccwgjyjlb_list_daily WHERE pid IN ($gjgdblccwgjyjlb_id)"
    ...
    mkkjccwgjyjlb_id="SELECT DISTINCT id FROM rep_mkkjccwgjyjlb_daily WHERE docid IN (${DOC_IDS})"
    mkkjccwgjyjlb_pid="SELECT DISTINCT id FROM rep_mkkjccwgjyjlb_list_daily WHERE pid IN ($mkkjccwgjyjlb_id)"
    ...
    clhljyjlb_id="SELECT DISTINCT id FROM rep_clhljyjlb_daily WHERE docid IN (${DOC_IDS})"
    clhljyjlb_pid="SELECT DISTINCT id FROM rep_clhljyjlb_list_daily WHERE pid IN ($clhljyjlb_id)"
    ...
    cljzjyjlb_id="SELECT DISTINCT id FROM rep_cljzjyjlb_daily WHERE docid IN (${DOC_IDS})"
    cljzjyjlb_pid="SELECT DISTINCT id FROM rep_cljzjyjlb_list_daily WHERE pid IN ($cljzjyjlb_id)"
    ...
    bzhbpbjjyjlba_id="SELECT DISTINCT id FROM rep_bzhbpbjjyjlba_daily WHERE docid IN (${DOC_IDS})"
    bzhbpbjjyjlba_pid="SELECT DISTINCT id FROM rep_bzhbpbjjyjlba_list_daily WHERE pid IN ($bzhbpbjjyjlba_id)"
    ...
    bzjjgjyjzb_id="SELECT DISTINCT id FROM rep_bzjjgjyjzb_daily WHERE docid IN (${DOC_IDS})"
    bzjjgjyjzb_pid="SELECT DISTINCT id FROM rep_bzjjgjyjzb_list_daily WHERE pid IN ($bzjjgjyjzb_id)"
    ...
    ntyclrtjyjlb_id="SELECT DISTINCT id FROM rep_ntyclrtjyjlb_daily WHERE docid IN (${DOC_IDS})"
    ntyclrtjyjlb_pid="SELECT DISTINCT id FROM rep_ntyclrtjyjlb_list_daily WHERE pid IN ($ntyclrtjyjlb_id)"
    ...
    hjgxjcjlb_id="SELECT DISTINCT id FROM rep_hjgxjcjlb_daily WHERE docid IN (${DOC_IDS})"
    hjgxjcjlb_pid="SELECT DISTINCT id FROM rep_hjgxjcjlb_list_daily WHERE pid IN ($hjgxjcjlb_id)"
    ...
    sbjwjyjzjlb_id="SELECT DISTINCT id FROM rep_sbjwjyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    sbjwjyjzjlb_pid="SELECT DISTINCT id FROM rep_sbjwjyjzjlb_list_daily WHERE pid IN ($sbjwjyjzjlb_id)"
    ...
    rwdxsyjzb_id="SELECT DISTINCT id FROM rep_rwdxsyjzb_daily WHERE docid IN (${DOC_IDS})"
    ...
    jzlclsgqyjzjlb_id="SELECT DISTINCT id FROM rep_jzlclsgqyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    ...
    jzlclsggcyjzjlb_id="SELECT DISTINCT id FROM rep_jzlclsggcyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    ...
    jzlclsghyjzjlb_id="SELECT DISTINCT id FROM rep_jzlclsghyjzjlb_daily WHERE docid IN (${DOC_IDS})"
    ...
    rtjxyzsyjzb_id="SELECT DISTINCT id FROM rep_rtjxyzsyjzb_daily WHERE docid IN (${DOC_IDS})"
    ...
    cdjghqtsyjcjzb_id="SELECT DISTINCT id FROM rep_cdjghqtsyjcjzb_daily WHERE docid IN (${DOC_IDS})"
    ...
    rep_ycllh_id="SELECT DISTINCT id FROM rep_ycllh_daily WHERE docid IN (${DOC_IDS})"
    rep_ycllh_pid="SELECT DISTINCT id FROM rep_ycllh_list_daily WHERE pid IN ($rep_ycllh_id)"
    ...
    rep_yzsygzj_id="SELECT DISTINCT id FROM rep_yzsygzj_daily WHERE docid IN (${DOC_IDS})"
    ...
    rep_sqcqzzxn_id="SELECT DISTINCT id FROM rep_sqcqzzxn_daily WHERE docid IN (${DOC_IDS})"
    export_table "rep_sqcqzzxn_daily" "id IN ($rep_sqcqzzxn_id)" 0 #报表-射汽抽汽装置性能试验见证表

    rep_jxyzsyjzb_id="SELECT DISTINCT id FROM rep_jxyzsyjzb_daily WHERE docid IN (${DOC_IDS})"
    export_table "rep_jxyzsyjzb_daily" "id IN ($rep_jxyzsyjzb_id)" 0 #报表-机械运转试验见证表

    rep_wfsysjsc_id="SELECT DISTINCT id FROM rep_wfsysjsc_daily WHERE docid IN (${DOC_IDS})"
    ...
    rep_wfsysj_id="SELECT DISTINCT id FROM rep_wfsysj_daily WHERE docid IN (${DOC_IDS})"
    ...
    rep_dxfdj_id="SELECT DISTINCT id FROM rep_dxfdj_daily WHERE docid IN (${DOC_IDS})"
    ...
    rep_lgysj_id="SELECT DISTINCT id FROM rep_lgysj_daily WHERE docid IN (${DOC_IDS})"
    ...
    rep_gcjyjz_id="SELECT DISTINCT id FROM rep_gcjyjz_daily WHERE docid IN (${DOC_IDS})"
    ...
    rep_lxsy_id="SELECT DISTINCT id FROM rep_lxsy_daily WHERE docid IN (${DOC_IDS})"
    rep_lxsy_pid="SELECT DISTINCT id FROM rep_lxsy_list_daily WHERE pid IN ($rep_lxsy_id)"
    ...
    rep_tssyjz_id="SELECT DISTINCT id FROM rep_tssyjz_daily WHERE docid IN (${DOC_IDS})"
    ...
    rep_xssyjz_id="SELECT DISTINCT id FROM rep_xssyjz_daily WHERE docid IN (${DOC_IDS})"
    export_table "rep_xssyjz_daily" "id IN ($rep_xssyjz_id)" 0 #报表-型式试验见证表
    export_table "rep_xssyjz_list_daily" "pid IN ($rep_xssyjz_id)" 0

    rep_ccjyjz_id="SELECT DISTINCT id FROM rep_ccjyjz_daily WHERE docid IN (${DOC_IDS})"
    rep_ccjyjz_pid="SELECT DISTINCT id FROM rep_ccjyjz_list_daily WHERE pid IN ($rep_ccjyjz_id)"
    ...
    rep_hfhxcf_id="SELECT DISTINCT id FROM rep_hfhxcf_daily WHERE docid IN (${DOC_IDS})"
    ...
    rep_hdchxcf_id="SELECT DISTINCT id FROM rep_hdchxcf_daily WHERE docid IN (${DOC_IDS})"
    ...
    rep_cpwgjcjlb_id="SELECT DISTINCT id FROM rep_cpwgjcjlb_daily WHERE docid IN (${DOC_IDS})"
    rep_cpwgjcjlb_pid="SELECT DISTINCT id FROM rep_cpwgjcjlb_list_daily WHERE pid IN ($rep_cpwgjcjlb_id)"
    ...
    #统计报表-报表
    STATIC_EQUIPMENT_PARENT_ID="SELECT DISTINCT id FROM rep_raw_material_outsourcing_state_static_equipment WHERE docid IN (${DOC_IDS})"
    ...
    STATIC_BOARD_PARENT_ID="SELECT DISTINCT id FROM rep_raw_material_outsourcing_state_static_board WHERE docid IN (${DOC_IDS})"
    ...
    STATIC_HET_PARENT_ID="SELECT DISTINCT id FROM rep_raw_material_outsourcing_state_static_het WHERE docid IN (${DOC_IDS})"
    ...
    STATIC_FORGING_PARENT_ID="SELECT DISTINCT id FROM rep_raw_material_outsourcing_state_static_forging WHERE docid IN (${DOC_IDS})"
    ...
    STATIC_WELD_PARENT_ID="SELECT DISTINCT id FROM rep_raw_material_outsourcing_state_static_weld WHERE docid IN (${DOC_IDS})"
    ...
    FM_PARENT_ID="SELECT DISTINCT id FROM rep_fm_wxwg_equipment_wxwg WHERE docid IN (${DOC_IDS})"
    ...
    FM_BC_PARENT_ID="SELECT DISTINCT id FROM rep_fm_wxwg_equipment_bc WHERE docid IN (${DOC_IDS})"
    ...
    FM_CJ_PARENT_ID="SELECT DISTINCT id FROM rep_fm_wxwg_equipment_cj WHERE docid IN (${DOC_IDS})"
    ...
    FM_DJ_PARENT_ID="SELECT DISTINCT id FROM rep_fm_wxwg_equipment_dj WHERE docid IN (${DOC_IDS})"
    ...
    RAW_EQUIPMENT_PARENT_ID="SELECT DISTINCT id FROM rep_raw_material_outsourcing_state_rotating_equipment WHERE docid IN (${DOC_IDS})"
    ...
    RAW_MAINPARTS_PARENT_ID="SELECT DISTINCT id FROM rep_raw_material_outsourcing_state_rotating_mainparts WHERE docid IN (${DOC_IDS})"
    ...
    WXWG_EQUIPMENT_PARENT_ID="SELECT DISTINCT id FROM rep_wxwg_equipment WHERE docid IN (${DOC_IDS})"
    ...
    LG_WXWG_EQUIPMENT_PARENT_ID="SELECT DISTINCT id FROM rep_lg_wxwg_equipment WHERE docid IN (${DOC_IDS})"
    ...
    CL_EQUIPMENT_PARENT_ID="SELECT DISTINCT id FROM rep_cl_equipment WHERE docid IN (${DOC_IDS})"
    ...
    GJG_EQUIPMENT_PARENT_ID="SELECT DISTINCT id FROM rep_gjg_equipment WHERE docid IN (${DOC_IDS})"
    ...
    JHG_EQUIPMENT_PARENT_ID="SELECT DISTINCT id FROM rep_jhg_wxwg_equipment WHERE docid IN (${DOC_IDS})"
    ...
    DQ_WXWG_EQUIPMENT_PARENT_ID="SELECT DISTINCT id FROM rep_dq_wxwg_equipment WHERE docid IN (${DOC_IDS})"
    ...
    DQ_LBJ_WXWG_EQUIPMENT_PARENT_ID="SELECT DISTINCT id FROM rep_dq_lbj_equipment WHERE docid IN (${DOC_IDS})"
    ...
    #放行单
    SYS_RELEASE_NOTE_ID="SELECT DISTINCT id FROM sys_release_note WHERE fid IN (${DISPATCH_IDS}) AND del_flag=0"
    ...
    echo -e "\n==> Compressing and encrypting: ${EXPORT_DIR}"
    if command -v pigz >/dev/null 2>&1; then
        (cd "$(dirname "$EXPORT_DIR")" && tar -I pigz -cf - "$(basename "$EXPORT_DIR")") | \
    ...
    if [ $? -eq 0 ]; then
        echo "==> Encrypted archive created: ${EXPORT_DIR}.tar.gz.enc"
        rm -rf "$EXPORT_DIR"
        echo "==> Removed original export directory: ${EXPORT_DIR}"
        ls -lh "${EXPORT_DIR}.tar.gz.enc"
    else
        echo "==> Compression/encryption failed"
        exit 1
    fi
...
do_decrypt() {
    ...
    if [ -z "$ENCRYPTED_FILE" ]; then
        echo "Usage: $0 decrypt <encrypted-file.tar.gz.enc>"
        exit 1
    fi
    if [ ! -f "$ENCRYPTED_FILE" ]; then
        echo "ERROR: file does not exist: $ENCRYPTED_FILE"
        exit 1
    fi
    if ! command -v tar >/dev/null 2>&1 || ! command -v openssl >/dev/null 2>&1; then
        echo "ERROR: tar or openssl command not found"
        exit 1
    fi
    ...
    if [[ "$BASENAME" == *.tar.gz.enc ]]; then
        ...
        echo "ERROR: file extension must be .tar.gz.enc"
        exit 1
    fi

    if [ -d "$OUTPUT_DIR" ]; then
        echo "WARN: output directory already exists: $OUTPUT_DIR"
    ...
    if openssl enc -d -aes-256-cbc -salt -pass pass:"$ENCRYPT_PASSWORD" -in "$ENCRYPTED_FILE" | tar xz --strip-components=1 -C "$OUTPUT_DIR"; then
        echo "==> Decryption completed: $OUTPUT_DIR"
        ls -lh "$OUTPUT_DIR"
    else
        echo "==> Decryption failed; check password or file integrity"
        exit 1
    fi
}
...
case "$1" in
    export)
        if [ -z "$2" ]; then
            echo "Usage: $0 export <PROJECT_ID>"
            exit 1
        fi
        export_project "$2"
        ;;
    export_contract)
        if [ -z "$2" ]; then
            echo "Usage: $0 export_contract <CONTRACT_NO>"
            exit 1
        fi
        export_contract "$2"
        ;;
    ...
    *)
        echo "Usage:"
        echo "  Export by project:  $0 export <PROJECT_ID>"
        echo "  Export by contract: $0 export_contract <CONTRACT_NO>"
        echo "  Decrypt:            $0 decrypt <encrypted-file.tar.gz.enc>"
        exit 1
        ;;
esac
