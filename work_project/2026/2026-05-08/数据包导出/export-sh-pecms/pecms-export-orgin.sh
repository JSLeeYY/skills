...
export_contract() {
  local PROJECT_ID_PARAM=$1
  do_export "CONTRACT" "$PROJECT_ID_PARAM"
}
query_id_list() {
    local sql="$1"
...
#      DISPATCH_ID="SELECT id FROM project_dispatch WHERE projectId='$PARAM_ID'"
      DISPATCH_IDS=$(query_id_list "
              SELECT id
              FROM project_dispatch
              WHERE projectId = '$PARAM_ID'
          ")
      # 查询项目信息是否存在
      ...
      if [ -z "$PROJECT_INFO" ]; then
          echo -e "\n===== 错误 ====="
          echo "项目ID【$PARAM_ID】不存在，数据库中无此记录！"
          echo "===============\n"
          exit 1
      fi
      # 获取项目名称
...
#      DISPATCH_ID="SELECT id FROM project_dispatch WHERE supervisor_contract_no='$PARAM_ID'"
        DISPATCH_IDS=$(query_id_list "
                      SELECT id
                      FROM project_dispatch
                      WHERE supervisor_contract_no = '$PARAM_ID'
                  ")
      # 根据合同号查询合同信息是否存在
      local PROJECT_INFO=$(mysql --defaults-extra-file=/root/.my.cnf -se "
          SELECT id, code FROM supervision_department_contract
          WHERE code='$PARAM_ID'
          LIMIT 1;
      ")
      # 根据ID未查询到项目数据
      if [ -z "$PROJECT_INFO" ]; then
          echo -e "\n===== 错误 ====="
          echo "合同【$PARAM_ID】不存在，数据库中无此记录！"
          echo "===============\n"
          exit 1
      fi
      PROJECT_NAME="合同号_$PARAM_ID"
    else
      echo "参数错误"
      exit 1
    fi
...
#            echo "  -> 无数据，删除空文件：${TABLE_NAME}.sql"
    ...
    # 导出业务表
    if [ "$TYPE" = "PROJECT" ]; then
      export_table "project_management" "id='$PARAM_ID'" 0
      export_table "project_apparatus" "project_id='$PARAM_ID'" 0
      export_table "project_dispatch" "projectId='$PARAM_ID'" 0
    elif [ "$TYPE" = "CONTRACT" ]; then
      export_table "supervision_department_contract" "code='$PARAM_ID'" 0
      export_table "project_dispatch" "supervisor_contract_no='$PARAM_ID'" 0
      export_table "project_management" "id IN (SELECT projectid FROM project_dispatch WHERE supervisor_contract_no='$PARAM_ID')" 0
      export_table "project_apparatus" "project_id IN (SELECT projectid FROM project_dispatch WHERE supervisor_contract_no='$PARAM_ID')" 0
    else
      echo "参数错误"
      exit 1
    fi
...
case "$1" in
    ...
    export_contract)
        if [ -z "$2" ]; then
            echo "用法: $0 export_contract <CONTRACT_NO>"
            exit 1
        fi
        export_contract "$2"
        ;;
    ...
    *)
        echo "用法:"
        echo "  按项目导出并加密: $0 export <PROJECT_ID>"
        echo "  按合同导出并加密: $0 export_contract <CONTRACT_NO>"
