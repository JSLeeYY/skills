...
GROUP_CONCAT_MAX_LEN="${GROUP_CONCAT_MAX_LEN:-67108864}"
...
mysql_exec() {
    mysql --defaults-extra-file="$MYSQL_CNF" --default-character-set="$MYSQL_CHARSET" "$@"
}
...
resolve_database_name() {
    ...
    MYSQL_DATABASE=$(mysql_exec -N -B -e "SELECT DATABASE();")
    if [ -z "$MYSQL_DATABASE" ] && [ -f "$MYSQL_CNF" ]; then
        MYSQL_DATABASE=$(awk -F= '/^[[:space:]]*database[[:space:]]*=/{gsub(/[[:space:]]/,"",$2); print $2; exit}' "$MYSQL_CNF")
    fi

    [ -n "$MYSQL_DATABASE" ] || die "无法从 ${MYSQL_CNF} 解析默认数据库名"
    printf '%s' "$MYSQL_DATABASE"
}
...
do_export() {
    ...
    local PROJECT_ID=$1
    local PROJECT_ID_ESCAPED
    PROJECT_ID_ESCAPED=$(sql_quote "$PROJECT_ID")
    ...
    PROJECT_EXISTS=$(mysql_exec -N -B -e "
        SELECT COUNT(*)
        FROM project_management
        WHERE id='${PROJECT_ID_ESCAPED}'
        LIMIT 1;
    ")

    if [ -z "$PROJECT_EXISTS" ] || [ "$PROJECT_EXISTS" = "0" ]; then
        echo -e "\n===== 错误 ====="
    ...
    # 保留为子查询 SQL，不在 shell 中物化为超长 ID 列表，
    # 避免大项目导出时把超长 WHERE 直接塞进 mysqldump 参数导致 "Argument list too long"。
    DISPATCH_IDS="
        SELECT id
        FROM project_dispatch
        WHERE projectId = '${PROJECT_ID_ESCAPED}'
    "
    ...
    to_export() {
        ...
        WHERE_CONDITION=$(normalize_sql "$2")
        ...
        if ! mysqldump_exec --where="$WHERE_CONDITION" "$DB_NAME" "$TABLE_NAME" >"$TMP_PATH" 2>"$ERR_PATH"; then
            echo "  -> 导出失败：${TABLE_NAME}" >&2
            cat "$ERR_PATH" >&2
            rm -f "$TMP_PATH" "$ERR_PATH"
            return 1
        fi

        if ! grep -q '^INSERT INTO ' "$TMP_PATH"; then
            echo "  -> 无数据，删除空文件：${TABLE_NAME}.sql"
            rm -f "$TMP_PATH" "$ERR_PATH" "$FILE_PATH"
            return 0
        fi
    ...
    export_table() {
        ...
        local SYS_FILE_FLAG=${3:-0}

        to_export "$TABLE_NAME" "$WHERE_CONDITION"

        if [ "$SYS_FILE_FLAG" -eq 1 ]; then
            register_file_bizid_source "$TABLE_NAME" "$WHERE_CONDITION"
        fi
    }

    build_file_bizid_union_sql() {
        local union_sql=""
        local sql
    ...
    copy_attachments_fast() {
        local BIZ_UNION_SQL=$1
        local ALL_PATHS_FILE="${EXPORT_DIR}/.all_attachment_paths.txt"
        local EXISTING_PATHS_FILE="${EXPORT_DIR}/.existing_attachment_paths.txt"
        local ALL_PATHS_FILE_ABS="${PWD}/${ALL_PATHS_FILE#./}"
        local EXISTING_PATHS_FILE_ABS="${PWD}/${EXISTING_PATHS_FILE#./}"
        ...
        mysql_exec -N -B -e "
            ...
            WHERE u.biz_id IN (
                SELECT id
                FROM (${BIZ_UNION_SQL}) export_biz_ids
            );
        " >"$ALL_PATHS_FILE"
        ...
        if [ -s "$EXISTING_PATHS_FILE" ]; then
            sort -u "$EXISTING_PATHS_FILE" -o "$EXISTING_PATHS_FILE"
            mkdir -p "$BACKUP_FILE_DIR"
            (
                cd "$FILE_ROOT" &&
                tar -cf - --files-from "$EXISTING_PATHS_FILE_ABS"
            ) | (
                cd "$BACKUP_FILE_DIR" &&
                tar -xf -
            )
        ...
        rm -f "$ALL_PATHS_FILE_ABS" "$EXISTING_PATHS_FILE_ABS"
    }

    export_by_ids() {
        local BIZ_UNION_SQL
        BIZ_UNION_SQL=$(build_file_bizid_union_sql)

        if [ -z "$BIZ_UNION_SQL" ]; then
            echo "==> No business ID"
            return 0
        fi

        to_export "sys_file_upload" "
            biz_id IN (
                SELECT id
                FROM (${BIZ_UNION_SQL}) export_biz_ids
            )
        ...
        echo -e "\n==> 开始复制附件..."
        copy_attachments_fast "$BIZ_UNION_SQL"
        echo -e "\n==> 附件导出完成！"
    }
...
#    export_table "dispatch_execution_actual_user" "dispatch_execution_id IN ($DISPATCH_EXECUTION_ID)" 0
    ...
    # 压缩加密
    echo -e "\n==> 正在使用加密压缩: ${EXPORT_DIR}"
    if (cd "$(dirname "$EXPORT_DIR")" && tar czf - "$(basename "$EXPORT_DIR")") | \
        openssl enc -aes-256-cbc -salt -pass pass:"$ENCRYPT_PASSWORD" -out "${EXPORT_DIR}.tar.gz.enc"; then
