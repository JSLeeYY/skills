...
# System Mapping
SYSTEM_MAPPING = {
    "执行管理与经营分析需求表.xlsx": "执行管理与经营分析系统",
    "预评价需求.xlsx": "PECMS系统 - 预评价模块",
    "后评价需求.xlsx": "PECMS系统 - 后评价模块",
    "数字信息平台优化提升需求评估 - 可读.xlsx": "数字信息平台优化提升",
}
...
def add_heading(doc, text, level):
    ...
    run.text = text
    set_font(run, size=16 if level==1 else 14, bold=True, color=RGBColor(0, 51, 102)) # Dark Blue for headers
        ...
        for j, cell_text in enumerate(row_cells):
            if j < len(row.cells):
                ...
                if i == 0:
                    ...
                    # cell._tc.get_or_add_tcPr().append(parse_xml(r'<w:shd {} w:fill="E7E6E6"/>'.format(nsdecls('w')))) # Simple background
...
def process_word_report(doc, filename, data):
    ...
    if comments:
        doc.add_heading("批注/修改意见", level=3)
        for c in comments:
            author = c.get("A", "Unknown")
            cmt_text = c.get("T", "")
            p = doc.add_paragraph()
            run_auth = p.add_run(f"【{author}】: ")
...
def main():
    ...
    run = title.add_run("数字一体化平台需求完整分析")
    ...
    # 2. Reports (Word Docs in subfolder)
    # The extraction keys for word docs are basenames like "PECMS平台监理周报格式修改模板.docx"
    
    doc.add_page_break()
    add_heading(doc, "现有报告分析与批注", level=1)
    
    for filename, content in data.items():
        if filename in processed_files:
            continue
        
        # Identify if it is a report (has Comments/Text structure)
        if isinstance(content, dict) and ("Text" in content or "Comments" in content):
            ...
            # Could be an unmapped Excel file
            add_heading(doc, f"其他文件: {filename}", level=2)
