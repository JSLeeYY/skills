...
# Common helpers for modal templates to reduce size
page_container = """
<div>
  <div class="page-title-row"><h1>{icon} {title}</h1><span class="role-badge">{{ role }}</span></div>
"""

# I need to insert ALL modules from V11 precisely without leaving any out.

# 1. 领导大屏 (DashboardPage) - Keep exactly what we had, it was good.
comp_dashboard = """
const DashboardPage = {
...
with open(os.path.join(base, 'script_gen_c1.py'), 'w', encoding='utf-8') as f:
    f.write(f'content = """{header}{comp_dashboard}{comp_contract}{comp_income}{comp_execution}{comp_workhour}"""\nopen("components_part1.js", "w", encoding="utf-8").write(content)')
